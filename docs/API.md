...
> Изменение `listen_port` / `listen_host` применяется сразу (hot-reload). Ответ может содержать `"listen_rebound": true`.

---

### `POST /api/v1/service/uninstall`

Останавливает и отключает systemd-сервис на хосте. Требует `management.allow_service_uninstall: true`.
...
Ответ `202 Accepted` (если удаление запланировано):

```json
{
  "scheduled": true,
  "message": "Сервис \"phantom-proxy\" будет остановлен через несколько секунд"
}
```

Если `systemctl` недоступен, вернётся `200` с полем `command` для ручного запуска.

Ошибки:
- `403` — `allow_service_uninstall` выключен
- `400` — неверная фраза подтверждения
...
| `/ui/service/uninstall` | Удаление systemd-сервиса (POST, danger zone) |
...
# Удалить сервис (нужен allow_service_uninstall: true)
curl -s -X POST -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
